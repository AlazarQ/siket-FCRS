        ...
        <x-bladewind::card reduce_padding="true">
            ...
            <div class="text-right">
                <x-bladewind::button has_spinner="true" name="save-user" onclick="showButtonSpinner('.save-user')">
                    <a href="{{ route('settings.currency.create') }}" class="btn btn-primary">Add New</a>
                </x-bladewind::button>
            </div><br>
            ...
            <x-bladewind::table selectable="flase" divider="thin" name="branch_list" searchable="true" celled="false"
                ...
                @foreach ($currencyList as $currency)
                    <tr>
                        <td class="md:hidden">{{ $currency->id }}</td>
                        <td class="text-left">{{ $currency->shortCode }}</td>
                        <td class="text-left">{{ $currency->description }}</td>
                        <td class="text-left">{{ $currency->status }}</td>
